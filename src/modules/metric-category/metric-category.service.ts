...
import MetricCategoryMetadata, { MetricCategoryMetadataUpdateAttributes } from "../../../database/models/metric-category-metadata.sequelize";
...
import {
    MetricCategoryMetadataCreateBodyInterface
} from "../../interfaces/metric-category-metadata-create-body.interface";
...
export class MetricCategoryService {
    ...
    public async update(metricCategoryId: any, params: MetricCategoryUpdateBodyInterface): Promise<MetricCategory> {
        return MetricCategory.sequelize.transaction(async (transaction) => {
            ...
            const updatedMetricCategory = await this.metricCategoryRepository.update(
                metricCategoryId,
                omitBy(
                    {
                        title: params.title,
            ...
            await this.metricCategoryMetadataRepository.update(
                {metricCategoryId},
                omitBy(
                    {
                        title: params.title,
                    ...
                    isUndefined
                ) as MetricCategoryMetadataUpdateAttributes,
                {transaction}
            );

            return updatedMetricCategory;
        });
    }
}